...
- Resumen CMS renderiza la fecha de ultima actualizacion como texto estable calculado en servidor para evitar diferencias de hidratacion entre SSR y navegador.
- Barra superior de `Mi sitio` compacta: en lugar de mostrar enlace y cuatro botones siempre abiertos, deja visible pagina actual y publicar; enlace, copiar, abrir pagina y guardar sin publicar quedan en `Enlace y opciones`.
...
- 2026-07-26: correccion defensiva de hidratacion al entrar a `Hostlyc Clone Test`; `repos/cms npm run lint/build` pasan y Playwright limpio en `http://localhost:4300` no reporta errores de hidratacion. Consola solo muestra mensajes normales de desarrollo (`React DevTools` y `[HMR] connected`). Capturas: `logs/screenshots/hydration-repro/hostlyc-summary-desktop-localhost-after-select.png` y `logs/screenshots/hydration-repro/hostlyc-summary-mobile-localhost-after-select.png`.
- 2026-07-26: barra de enlace en `Mi sitio` compactada despues de hallazgo visual en mobile; Playwright validado en desktop `1366x768` y mobile `390x844`, sin overflow horizontal ni errores de consola. Capturas: `logs/screenshots/cms-unified-editor/desktop-1366-compact-link-options.png` y `logs/screenshots/cms-unified-editor/mobile-390-compact-link-options.png`.
